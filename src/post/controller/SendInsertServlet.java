...
/**
 * Servlet implementation class SendInsertServlet
 */
@WebServlet("/sinsert")
public class SendInsertServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SendInsertServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//보내기용 컨트롤러
				...
				int result = new PostService().sendInsertPost(post);
				...
				if(result>0) {
					...
					response.setContentType("text/html; charset=UTF-8");
			           ...
			           out.println("<script>alert('제목, 받는사람, 내용은 필수 입력 항목 입니다.');document.location.href='views/post/postWriteForm.jsp';</script>");
			           out.flush();
			           out.close();
				}
			}else {
		         response.setContentType("text/html; charset=UTF-8");
		           PrintWriter out = response.getWriter();
		           out.println("<script>alert('금지어가 포함된 편지는 보낼 수 없습니다.');document.location.href='views/post/postWriteForm.jsp';</script>");
		           out.flush();
		           out.close();
		      }
