...
/**
 * Servlet implementation class MydiaryInsertServlet
 */
@WebServlet("/mdinsert")
public class MydiaryInsertServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MydiaryInsertServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = new DiaryService().insertMyDiary(diary);

		if (result > 0) {
			response.sendRedirect("mdlist?page=1");
		} else {
			response.setContentType("text/html; charset=UTF-8");
	        PrintWriter out = response.getWriter();
	        out.println("<script>alert('내 일기 등록에 실패하였습니다.'); history.go(-1);</script>");
	        out.flush();
	        out.close();
		}
