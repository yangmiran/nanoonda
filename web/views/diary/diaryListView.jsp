...
<body>
	...
	<div class="writecon">
	<div align="center">
		<a href="/nnd/views/diary/diaryWriteForm.jsp" class="todaywrite">오늘의 일기 쓰기</a>
	</div>
	<div id="search" align="center">
		<form action="/nnd/dsearch" method="post">
			<input type="text" name="keyword" class="searchblock">
			<input class="bottomb" type="submit" value="제목검색">
		</form>
	</div>
    ...
    <% for(Diary d : list){ %>
		<tr>
			<td>
				<a href="/nnd/ddetail?dno=<%= d.getDiaryNo() %>&page=<%= currentPage %>"><%= d.getDiaryTitle() %></a>
	...
	<div id="page" style="text-align:center;">
		...
		<% } else { %>
			<span><a href="/nnd/dlist?page=1" class="ti-angle-double-left"></a></span>
		<% } %>
		...
		<% for(int p = startPage; p <= endPage; p++){
			...
			<% } else { %>
				<span class="number"><a href="/nnd/dlist?page=<%= p %>"><%= p %></a></span>
			<% } %>
		<% } %>
		...
		<% } else { %>
			<span><a href="/nnd/dlist?page=<%= maxPage %>" class="ti-angle-double-right"></a></span>
		<% } %>
	</div>
