...
public class DiaryUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String openOk = mrequest.getParameter("openOk");
		...
		if(checkResult == false) { //금지어 통과시
			...
			//서비스 객체 생성해서 Diary 객체 전달하고 처리된 결과 받기
			...
			if (result > 0) {
				if(openOk.equals("Y")) {
					...
					response.sendRedirect("mdlist?page=" + currentPage);
				}				
			} else {
				response.setContentType("text/html; charset=UTF-8");
		        PrintWriter out = response.getWriter();
		        out.println("<script>alert('일기 수정에 실패하였습니다.'); history.go(-1);</script>");
		        out.flush();
		        out.close();
			}
		} else {
